...
def check_nick(phenny, channel, nick):
    seen = False
    sqlitedata = {
        'channel': channel,
        'nick': nick,
...
def on_join(phenny, input):
    ...
    nick = input.nick
    if check_nick(phenny, channel, nick):
        return
    else:
        party(phenny, input)
        seen_nick(phenny, channel, nick)
on_join.rule = r'.*'
...
def party(phenny, input):
    ...
    if nick in vip:
        ...
        phenny.action(input.sender, 'brings out the party cannon for {0}!'.format(nick))
...
def seen_nick(phenny, channel, nick):
    ...
    c.execute('''insert or replace into nicks values(:channel, :nick);''', sqlitedata)
    c.close()
    seen_nick.conn.commit()
